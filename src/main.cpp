...
/*
 ...
 Simon Says is a memory game. Start the game by pressing one of the four
 buttons. When a button lights up, press the button, repeating the sequence. The
 sequence will get longer and longer. The game is won after 13 rounds.
...
// Returns a '1' bit in the position corresponding to CHOICE_RED, CHOICE_GREEN,
// etc.
...
// This is just a unique (annoying) sound we came up with, there is no magic to
// it
...
boolean play_battle(void) {
  ...
  while (1) // Loop until someone fails
  {
    ...
    delay(100); // Give the user an extra 100ms to hand the game to the other
                // player
  }
...
int melody[] = {NOTE_G4, NOTE_A4, 0,       NOTE_C5, 0, 0,       NOTE_G4,
                0,       0,       0,       NOTE_E4, 0, NOTE_D4, NOTE_E4,
                NOTE_G4, 0,       NOTE_D4, NOTE_E4, 0, NOTE_G4, 0,
                0,       NOTE_D4, 0,       NOTE_E4, 0, NOTE_G4, 0,
                NOTE_A4, 0,       NOTE_C5, 0};

int noteDuration = 115; // This essentially sets the tempo, 115 is just about
                        // right for a disco groove :)
...
// This function is activated when user holds bottom right button during power
// up
void play_beegees() {
  ...
  setLEDs(CHOICE_RED | CHOICE_GREEN |
          CHOICE_BLUE); // Turn on the other LEDs until you release button
  ...
  digitalWrite(BUZZER1, LOW); // setup the "BUZZER1" side of the buzzer to stay
                              // low, while we play the tone on the other pin.
...
void loop() {
  ...
  setLEDs(CHOICE_RED | CHOICE_GREEN | CHOICE_BLUE |
          CHOICE_YELLOW); // Turn all LEDs on
  delay(1000);
